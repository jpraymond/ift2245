...
class ServerThreads {
 ...
 public:
  ...
  static void processRequestNew(int threadID, int socketFD);
  void printAndSaveResults(const char* fileName);
  void signalFinishToClient();
	
 private:

  static boolean isNotSafe(int & id, int & Request []);
  static boolean isExcessive(int & id, int & Request []);
  static boolean isSmaller(const int& Left[], const int &Right[], int dim);
  static int parseCheckRequest(char & Buffer[], int& clientThreadId,  int& Request[]);
  static boolean isEqualToZero(int & Tab [], int dim);
  ...
  static boolean *ClientsReleased;
